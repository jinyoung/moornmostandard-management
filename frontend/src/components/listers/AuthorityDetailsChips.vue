<template>
    <div style="margin: 0 -15px 0 -15px;">
        <v-list two-line v-if="value.length > 0">
            <v-list-group v-model="isExpansion">
                <template v-slot:activator>
                    <v-list-item-content>
                        <v-list-item-title>AuthorityDetails</v-list-item-title>
                    </v-list-item-content>
                </template>
                ...
                <v-list-item v-for="(item, idx) in value" :key="idx">
                    <v-list-item-content>
                        ...
                        <v-list-item-subtitle>
                            menuCode :  {{item.MenuCode }} <br>
                            authType :  {{item.AuthType }} <br>
                            authCode :  {{item.AuthCode }} <br>
                            authMode :  {{item.AuthMode }} <br>
                        </v-list-item-subtitle>
                    </v-list-item-content>
        ...
        <AuthorityDetails v-if="editMode && tick" offline :isNew="true" :inList="true"
...
<script>
    import AuthorityDetails from '../AuthorityDetails.vue';

    export default {
        name: 'AuthorityDetailsManager',
        components: {
            AuthorityDetails
        },
        props: {
