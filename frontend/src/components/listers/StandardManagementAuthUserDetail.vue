<template>
    <v-card outlined>
        <v-card-title>
            AuthUser # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <String label="WorkerNo" v-model="item.workerNo" :editMode="editMode" @change="change" />
            </div>
            <div>
            ...
            <UserDetails offline label="UserDetails" v-model="item.userDetails" :editMode="false" :key="false" @change="change" />
        </v-card-text>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/authUsers/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
